...
class TimedArray(Function, Nameable):
    ...
    def _init_1d(self):
        ...
        def create_cuda_implementation(owner):
            ...
            K = _find_K(group_dt, dt)
            support_code = '''
            __device__ double timedarray_%NAME%(const double t, const int _num_values, const double* _values)
            {
            ...
            cuda_code = {'support_code': support_code,
                        'hashdefine_code': '''
            #define %NAME%(t) timedarray_%NAME%(t, _%NAME%_num_values, d_%NAME%_values)
